function resolve_values_paths( \
  list_dir_values_paths_array, result, \
  _cmd, _cmd2, _value_name, _i \
) {
  for (_i in list_dir_values_paths_array) {
    _cmd = "find " list_dir_values_paths_array[_i] " -name \"_*\""
...
function read_value_file( \
  ...
  while((getline _line < (recognised_value["path"])) > 0) {
    if (recognised_value["type"] == "inline") _value_lines = _value_lines _line
    else if (recognised_value["type"] == "pre") _value_lines = _value_lines escape_ml(_line) "\n"
    else if (recognised_value["type"] == "block") _value_lines = _value_lines _line "\n"
    else continue
  }
  close(recognised_value["path"])

  if (recognised_value["type"] == "pre") _value_lines = trim_last_char(_value_lines)
...
function substitute_with_recognized_values( \
  ...
  _recognised_value_lines, _fragment_out, _indent, _indent_match, _recognised_value_placeholder, _recognised_value, _empty_line, _i \
) {
  for (_recognised_value in recognised_values) {
    _recognised_value_lines = ""
    _read = 0

    split(fragment, _fragment_out_array, "\n")
    _fragment_out = ""
    _recognised_value_placeholder = "<!-- " _recognised_value " -->"

    for (_i in _fragment_out_array) {
      _empty_line = 0
      if (match(_fragment_out_array[_i], _recognised_value_placeholder)) {
        if (! _read) {
          _recognised_value_lines = read_value_file(values_index[_recognised_value])
          _read = 1
        }

        if (values_index[_recognised_value]["type"] == "block") {
          ...
          sub(_recognised_value_placeholder, indent_lines(trim_last_char(_recognised_value_lines), _indent), _fragment_out_array[_i])
        } else {
          sub(_recognised_value_placeholder, _recognised_value_lines, _fragment_out_array[_i])
        }
      }
...
function build_document( \
  list_dir_values_paths, path_root, url_path, \
  _fragment_out, _list_dir_values_paths_array, _values_index, _document_filename, _recognised_values \
) {
  split(list_dir_values_paths, _list_dir_values_paths_array, ":")

  resolve_values_paths(_list_dir_values_paths_array, _values_index)
  ...
  _fragment_out = remove_block_value_placeholder_lines(_fragment_out)
  ...
  if (! system("mkdir -p " path_root "/" url_path)) print _fragment_out >> path_root "/" url_path "/" _document_filename
  
  if (! system("test -d " _list_dir_values_paths_array[length(_list_dir_values_paths_array)] "/static")) {
    system("cp -r " _list_dir_values_paths_array[length(_list_dir_values_paths_array)] "/static/* " path_root "/" url_path "/")
  }
}

BEGIN {
  build_document(list_dir_values_paths, path_root, url_path)
